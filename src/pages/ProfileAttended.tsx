...
export const ProfileAttended = () => {
  ...
  return (
    <div className="max-w-3xl w-full mx-auto p-6 min-h-screen">
      ...
      <h2 className="text-2xl font-bold mt-4 mb-4">Min närvaro</h2>

      <div className="bg-white p-4 rounded-md shadow mb-4">
        <p className="text-sm text-gray-600">Möten sedan senaste utmärkelse</p>
        <p className="text-3xl font-bold mt-1">{sinceLastAchievementCount}</p>
      </div>

      <div className="bg-white p-4 rounded-md shadow">
        <h3 className="text-lg font-semibold mb-3">Möten deltagna</h3>

        {loading ? <p className="text-gray-500">Laddar...</p> : null}

        {!loading && events.length === 0 ? (
          <p className="text-gray-500">Inga detagna möten hittades.</p>
        ) : null}
