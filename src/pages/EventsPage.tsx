    ...
    return (
        <div className="p-6 w-full max-w-4xl md:max-w-5xl lg:max-w-6xl xl:max-w-[1100px] mx-auto min-h-screen">
            ...
            <div className="grid grid-cols-7 gap-1 border rounded-lg overflow-hidden">
                {WEEK_DAYS.map((wd) => (
                    <div key={wd} className="bg-gray-50 text-center py-2 text-sm font-medium">
                        {wd}
                    </div>
                ))}
                    ...
                    <React.Fragment key={wi}>
                        {week.map((date, di) => {
                            ...
                            return (
                                <div
                                    key={di}
                                    className={`min-h-[80px] md:min-h-[100px] lg:min-h-[120px] p-2 text-sm border-t border-l bg-white min-w-0 ${inMonth ? "" : "bg-gray-50 text-gray-400"
                                        }`}
                                    style={{ display: "flex", flexDirection: "column" }}
                                >
                                    <div className="flex items-start justify-between">
                                        <div className={`w-6 h-6 flex items-center justify-center rounded-full ${isToday ? "bg-green-600 text-white" : "text-gray-700"}`}>
                                            {date ? date.getDate() : ""}
                                        </div>
                                    </div>
                                        ...
                                        {!loading && !error && date && (() => {
                                            ...
                                            return (
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    {evs.length > 3 && (
                                                        <div className="text-xs text-gray-500">+{evs.length - 3} more</div>
                                                    )}
                                                </div>
                                            );
                                        })()}
